...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_GET['action']) && $_GET['action'] === 'mark_read') {
    ...
    try {
        ...
        error_log("Mark as Read Error (profile.php - patient): " . $e->getMessage());
...
try {
    ...
    $stmt_notif->close();
     ...
     error_log("Error fetching notifications for patient profile: " . $e->getMessage());
}
// === END NOTIFICATION FETCH ===


// ==========================================================
// 1. HANDLE FORM SUBMISSIONS FIRST (Yung code mo, walang binago dito)
// ==========================================================
$fullname_val = ''; 
$username_val = '';
$email_val = '';
// $specialty_val = ''; // Not needed for patient
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_info') {
    ...
    if (empty($new_fullname) || empty($new_username) || empty($new_email) || !filter_var($new_email, FILTER_VALIDATE_EMAIL)) {
        ...
        try {
            ...
            $check_stmt->bind_param("ssi", $new_email, $new_username, $patient_id);
            $check_stmt->execute();
            ...
            if ($check_result->num_rows > 0) {
                ...
                $update_stmt = $conn->prepare("UPDATE users SET fullname = ?, username = ?, email = ? WHERE id = ? AND role = 'patient'");
                if (!$update_stmt) throw new Exception("Prepare failed (update): " . $conn->error);
                $update_stmt->bind_param("sssi", $new_fullname, $new_username, $new_email, $patient_id);

                if ($update_stmt->execute()) {
                    $_SESSION['fullname'] = $new_fullname; 
                    $message = 'Success! Profile information updated successfully.';
                    $message_type = 'success';
                    $patient_name = $new_fullname; // Update header name agad
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'change_password') {
    ...
    if (empty($old_pass) || empty($new_pass) || empty($confirm_pass)) {
        ...
        try {
            $select_pass_stmt = $conn->prepare("SELECT password FROM users WHERE id = ? AND role = 'patient'");
            if (!$select_pass_stmt) throw new Exception("Prepare failed (select pass): " . $conn->error);
            $select_pass_stmt->bind_param("i", $patient_id);
            ...
            if ($user && password_verify($old_pass, $user['password'])) {
                ...
                $update_pass_stmt = $conn->prepare("UPDATE users SET password = ? WHERE id = ? AND role = 'patient'");
                if (!$update_pass_stmt) throw new Exception("Prepare failed (update pass): " . $conn->error);
                $update_pass_stmt->bind_param("si", $hashed_new_pass, $patient_id);
...
// ==========================================================
// 2. FETCH PATIENT DATA FOR DISPLAY (Yung code mo, walang binago dito)
// ==========================================================
if (!($message_type === 'success' && isset($_POST['action']) && $_POST['action'] === 'update_info')) {
    try {
        $stmt = $conn->prepare("SELECT fullname, username, email FROM users WHERE id = ? AND role = 'patient'"); // Inalis ang specialty
        ...
        if ($result->num_rows === 1) {
            ...
            $patient_name = $patient_data['fullname'] ?? $patient_name; 
            if (empty($fullname_val)) $fullname_val = htmlspecialchars($patient_data['fullname'] ?? '');
            if (empty($email_val)) $email_val = htmlspecialchars($patient_data['email'] ?? '');
            if (empty($username_val)) $username_val = htmlspecialchars($patient_data['username'] ?? ''); 
        } else {
            if (empty($message)) { 
                $message = 'Error: Patient data not found. Please relog.';
                $message_type = 'error';
            }
        }
        $stmt->close();
...
// Assign default values if still empty to prevent errors
$fullname_val = $fullname_val ?: '';
$email_val = $email_val ?: '';
...
<head>
  ...
  <title>CliniCare — Patient Profile</title>
...
<body>
  ...
  <header class="topnav">
    <div class="nav-inner">
      ...
      <div class="nav-right">
        ...
        <div class="profile" id="profileToggle"> <img src="../PICTURES/doc1.jpg" alt="Patient Avatar" class="avatar"> <span class="name"><?= htmlspecialchars($patient_name); ?></span>
          <i class="fa-solid fa-chevron-down caret"></i>
          <ul class="profile-dropdown" id="profileDropdown"> <li><a href="profile.php" class="active"><i class="fa-solid fa-user-pen"></i> Edit Profile</a></li> 
      ...
      <form id="infoForm" method="POST" action="profile.php">
        ...
        <div class="form-grid">
          ...
          <div class="form-group">
            ...
            <input type="text" id="roleDisplay" value="Patient" disabled style="background-color: #f3f4f6; color: #6b7280; cursor: not-allowed;"> </div>
          ...
          <div class="form-group">
            ...
            <input type="text" id="username" name="username" value="<?= $username_val ?>" required>
          </div>
        </div>
  ...
  <script>
    document.addEventListener("DOMContentLoaded", () => {
        ...
        if (notifBell && notifDropdown) {
            notifBell.addEventListener("click", (e) => {
                ...
                if (notifDropdown.classList.contains("show") && notifBadge && notifBadge.style.display !== 'none') {
                    ...
                    // 💡 Tinatawag ang profile.php na may API router
                    fetch('profile.php?action=mark_read', { 
                        method: 'POST',
                        headers: { 
        ...
        // --- PART B: PROFILE PAGE SPECIFIC JS (Password Validation, etc.) ---
       ...
       if (window.history.replaceState) {
            ...
            // 💡 AYOS: Check kung may laman 'yung message bago i-clear
